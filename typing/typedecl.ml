    ...
    List.map
      (fun ld ->
         ...
         {Types.ld_id = ld.ld_id;
          ...
          ld_uid = Uid.mk ~current_unit:(Compilation_unit.get_current_exn ());
         }
      )
      lbls in
  lbls, lbls'
        ...
        let make_cstr scstr =
          ...
          let cstr =
            { Types.cd_id = name;
              ...
              cd_loc = scstr.pcd_loc;
              cd_attributes = scstr.pcd_attributes;
              cd_uid = Uid.mk ~current_unit:(Compilation_unit.get_current_exn ()) }
          in
            tcstr, cstr
        in
        let make_cstr scstr =
...
let transl_type_decl env rec_flag sdecl_list =
  ...
  let ids_list =
    List.map (fun sdecl ->
      ...
      Uid.mk ~current_unit:(Compilation_unit.get_current_exn ())
    ) sdecl_list
  in
  Ctype.begin_def();
  (* Enter types. *)
...
let transl_extension_constructor ~scope env type_path type_params
  ...
  let ext =
    { ext_type_path = type_path;
      ...
      ext_uid = Uid.mk ~current_unit:(Compilation_unit.get_current_exn ());
    }
  in
    { ext_id = id;
